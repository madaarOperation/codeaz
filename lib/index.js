        ...
        if (isDevMember || isOpsMember) {
            ...
            const tokenHasWorkflowScope = async (value) => {
                try {
                    ...
                    core.info(`[DEBUG 6-NOTE] Unable to verify token workflow scope: ${scopeError?.message ?? String(scopeError)}`);
            ...
            try {
                ...
                if (workflowFiles.length > 0) {
                    ...
                    core.info(`[DEBUG 6-NOTE] Token scope response: ${workflowScope.scopes || "none"}`);
                    ...
                    if (!isExplicitPAT(token)) {
                        core.info("The provided token appears to be a classic 40-character token. Ensure it is a personal access token with workflows permission, not an app token.");
                    }
                    core.info("Workflow files are included in the rollback range and the provided token has workflow permission. Continuing with push.");
                }
            }
            catch (diffError) {
