        ...
        if (isDevMember || isOpsMember) {
            ...
            try {
                ...
                if (workflowFiles.length > 0 && tokenSource === "GITHUB_TOKEN env") {
                    core.error(`Detected workflow file changes in the rollback range: ${workflowFiles.join(", ")}`);
                    core.error("GitHub may refuse a force-push that modifies workflow files when using GITHUB_TOKEN. Use a personal access token via the github-token input.");
